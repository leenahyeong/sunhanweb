...
@WebServlet("/reservechild.do")
public class ReserveChildController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String userid = request.getParameter("userid"); // 마이페이지에서 받아온 파라미터 아동
		...
		// 검색 조건 들고옴
		String search_userid = request.getParameter("search_storeid"); // 아이디 검색(가게)
		...
		listObj.put("rv_userid", loginid);
		
		ReserveDAO dao = ReserveDAO.getInstance();

		// 한 화면에 15개의 게시글을 보여지게함
		// 페이지 번호는 총 5개, 이후로는 [다음]으로 표시
		int listCount = dao.ChildtotalCount(listObj);
		// 전체 페이지 수
		...
		if(loginid.equals(userid)) {
			...
			List<ReserveChildDTO> list = dao.reserveChildList(listObj);
		...
		String url = "reserve/reserveChild.jsp";
