...
<body>
	...
	<main id="main" class="container-flude">
		...
		<div class="container real_c">
			...
			<section class="auto">
					...
					<div class="card-body search_div">
						<table class="" id="search_table">
							<tbody>
								<tr>
									<th>아이디검색</th>
									<td><input type="text" class="search_text" name="search_userid"></td>
								</tr>
				...
				<div class="float-right mg8">
					<select class="search_select outline" name="sort">
						<option value="sort_rvrno">등록일순</option>
						<option value="sort_rvtime">방문예정순</option>
					</select>
				</div>
				<table class="table board_table listab reservelist">
					<thead>
						<tr>
							...
							<th style="width: 14%">예약자명</th>
							<th style="width: 27%">방문예정시간</th>
					...
					<tbody>
						<c:forEach var="lists" items="${list}" varStatus="i">
							...
							<tr>
								...
								<td class="center_it"><span>${lists.rv_userid}</span></td>
			...
			<div class="paging">
				<ul>
					<c:if test="${startPage > 1}">
						<li><a href="reservesupporter.do?&userid=${loginUserID}&page=${startPage-1}"> <span
					...
					<c:forEach var="paging" begin="${startPage}" end="${endPage}">
						<c:if test="${paging == spage}">
							<li class="p_active"><a href="reservesupporter.do?&userid=${loginUserID}&page=${paging}">${paging}</a>
							</li>
						</c:if>

						<c:if test="${paging != spage}">
							<li><a href="reservesupporter.do?&userid=${loginUserID}&page=${paging}">${paging}</a></li>
						</c:if>
					</c:forEach>

					<c:if test="${endPage != maxPage}">
						<li><a href="reservesupporter.do?&userid=${loginUserID}&page=${endPage+1}"> <span
	...
	$(document).ready(function(){
		...
		searchBtn.onclick = function() {
			...
			console.log("s" + start_date + "e" + end_date + start_date.length);
		};
